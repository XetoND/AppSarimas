...
export function Home({ posts }) {
    ...
    return (
      ...
      <main className={styles.main}>
        ...
        <map name="sarimasmap" className={styles.map}>
          <Link href={{ pathname: '/content/Detail', query: { id: 11} }}>
          ...
          <Link href={{ pathname: '/content/Detail', query: { id: 8} }}>
      ...
      <center>
        <div className={styles.grid}>
          {posts && posts.data.map(post =>(
            ...
            <a data-key={post.id}>
              <h2 className={styles.card}>{post.attributes.judul}</h2>
            </a>
            </Link>
          ))}
        </div>
